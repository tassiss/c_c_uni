program cond
    ...
    !use g_s
    implicit none      
    integer::i=2, n, status=0, show, cont=1, n_g, i_e, n_i!i=posição no vetor, n=número de nós
    ...
    allocate(b(1-n_g:n+n_g),stat=status) !aloca o vetor b utilizado para calcular a temperatura
    if (status/=0) then
        write(*,*) 'erro de alocação b'
        stop
    end if
    ...
    !#####CÁLCULO DO MÉTODO NUMÉRICO########   
        ...
        if (i_e==2)then !seleção para resolver méto explicito
            ...
            do i=1,n !a temperatura nas pontas da barra variam
                ...
                dkp=(d_k(i)+d_k(i+1))/2.0d0
                dkn=(d_k(i-1)+d_k(i))/2.0d0
                w(i)= b(i)+(((dt/((dx**2.0d0)))*((dkp*(b(i+1)-b(i)))-(dkn*(b(i)-b(i-1))))))/(rho*cp)!equação da tempratura na barra
            end do
        end if
